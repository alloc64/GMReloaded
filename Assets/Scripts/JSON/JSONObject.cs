...
namespace TouchOrchestra
{
    ...
    public class JSONObject : Nullable
    {
		...
		public JSONObject(Dictionary<string, float> dic)
		{
			type = Type.OBJECT;
            ...
            if (str != null)
            {
                ...
                if (str.Length > 0)
                {
                    ...
                    else
                    {
                        ...
                        type = Type.NUMBER;
                        if (!double.TryParse(str, NumberStyles.Number, CultureInfo.InvariantCulture, out n))
                        {
                            int token_tmp = 0;
                            /*
...
*/
        ...
        public void SetField(string name, int number)
        {
            if (HasField(name))
            {
                list.Remove(this[name]);
		...
		public int GetFieldi(string name)
		{
			if (type == JSONObject.Type.OBJECT)
				for (int i = 0; i < keys.Count; i++)
					if ((string)keys[i] == name)
						return (int)((JSONObject)list[i]).n;
			return 0;
		}

		public long GetFieldl(string name)
		{
			if (type == JSONObject.Type.OBJECT)
				for (int i = 0; i < keys.Count; i++)
					if ((string)keys[i] == name)
						return System.Convert.ToInt64(((JSONObject)list[i]).n);
			return 0;
		}

        public float GetFieldf(string name)
        {
            if (type == JSONObject.Type.OBJECT)
                for (int i = 0; i < keys.Count; i++)
                    if ((string)keys[i] == name)
                        return (float)((JSONObject)list[i]).n;
            return 0;
        }
